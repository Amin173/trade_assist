...
def _apply_liquidity_caps(
    ...
    for ticker in capped.index:
        ...
        is_buy = shares > 0.0
        if is_buy and min_adv_dollars > 0.0 and adv < min_adv_dollars:
            capped.loc[ticker] = 0.0
            touched += 1
            continue
...
def _enforce_min_trade_shares(
    ...
    threshold = max(float(min_trade_shares), 0.0)
    if threshold <= 0.0:
        return trade_shares
...
def run_policy(
    ...
    for ticker in tickers:
        if holdings.loc[ticker] > 0:
            ...
            if pd.notna(px0) and float(px0) > 0.0:
    ...
    for day in dates[:-1]:
        ...
        if cfg.risk_exit.stop_loss_pct > 0.0 or cfg.risk_exit.trailing_stop_pct > 0.0:
            for ticker in tickers:
                if holdings.loc[ticker] <= 0.0:
                    continue
        ...
        if do_rebalance:
            ...
            if eligible:
                ...
                if len(s) > 0:
                    ...
                    tradable = fill_px_eligible[fill_px_eligible > 0.0].dropna()
                    if len(tradable) > 0.0:
        ...
        for ticker in tickers:
            ...
            if trade_notional.loc[ticker] > 0.0:
                buy_slippage += slip
            else:
                sell_slippage += slip
        ...
        buy_scale = 1.0
        if available_for_buys <= 0.0:
            buy_scale = 0.0
        elif estimated_buy_cost > available_for_buys and estimated_buy_cost > 0.0:
            buy_scale = max(0.0, min(1.0, available_for_buys / estimated_buy_cost))

        if buy_scale < 1.0:
            buy_mask = trade_shares > 0.0
        ...
        for ticker in tickers:
            ...
            if prev_shares <= 0.0 and new_shares > 0.0:
                if pd.notna(fill) and float(fill) > 0.0:
                    entry_price.loc[ticker] = float(fill)
                    peak_since_entry.loc[ticker] = float(fill)
                elif pd.notna(day_px) and float(day_px) > 0.0:
                    entry_price.loc[ticker] = float(day_px)
                    peak_since_entry.loc[ticker] = float(day_px)
                continue

            if delta > 0.0 and pd.notna(fill) and float(fill) > 0.0:
                if pd.notna(prev_entry) and float(prev_entry) > 0.0:
